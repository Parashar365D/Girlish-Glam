...
useEffect(() => {
  const fetchProducts = async () => {
    try {
      const response = await fetch('https://girlish-glam-server.onrender.com/product/fetchproduct');
  ...
  const fetchCartItems = async () => {
    if (localStorage.getItem('auth-token')) {
      try {
        const response = await fetch('https://girlish-glam-server.onrender.com/cart/getcart', {
          method: 'GET',
          headers: {
...
const addToCart = async (productId, quantity = 1) => {
  if (localStorage.getItem('auth-token')) {
      try {
          const response = await fetch('https://girlish-glam-server.onrender.com/cart/addcart', {
              method: 'POST',
              headers: {
                  'Accept': 'application/json',
                  'auth-token': localStorage.getItem('auth-token'),
...
const removeFromCart = async (productId, quantity = 1) => {
  if (localStorage.getItem('auth-token')) {
    try {
      const response = await fetch('https://girlish-glam-server.onrender.com/cart/removecart', {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
          'auth-token': localStorage.getItem('auth-token'),
...
const clearCart = async (id) => {
  if (localStorage.getItem('auth-token')) {
    try {
      const response = await fetch('https://girlish-glam-server.onrender.com/cart/clearcart', {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
          'auth-token': localStorage.getItem('auth-token'),
